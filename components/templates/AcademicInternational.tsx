...
export const AcademicInternational: React.FC<TemplateProps> = ({ data, paperSize = "letter" }) => {
  ...
  return (
    <div
      className={`bg-white text-zinc-950 font-serif leading-normal w-full min-h-full selection:bg-zinc-100 ${
    ...
    >
      ...
      <header className="text-center pb-2 mb-2.5">
        ...
        {headline && (
          <p className="text-[10pt] italic text-zinc-700 mb-1 font-serif">
            {headline}
          </p>
        )}
        ...
        {contactItems.length > 0 && (
          <div className="text-[9pt] text-zinc-700 flex flex-wrap justify-center items-center gap-x-2.5 gap-y-0.5 font-sans">
            ...
            case "education":
              ...
              return (
                <section key="education" className="page-break-avoid">
                  <h2 className="text-[10.5pt] font-bold uppercase tracking-wider text-zinc-950 border-b border-zinc-400 pb-0.5 mb-2 font-sans">
                    {labels.education}
                  </h2>
                    ...
                    {education.map((edu) => (
                      <div key={edu.id} className="page-break-avoid">
                        ...
                        <div className="flex justify-between items-baseline text-[9.5pt]">
                          ...
                          {edu.location && (
                            <span className="text-[8.5pt] text-zinc-600 font-sans">
                              {edu.location}
                            </span>
                          )}
                        </div>
                    ...
                    {projects.map((proj) => (
                      <div key={proj.id} className="page-break-avoid">
                        <div className="flex justify-between items-baseline mb-0.5">
                          ...
                          {proj.start_date && (
                            <span className="text-[8.5pt] italic text-zinc-600 font-sans shrink-0">
                              {proj.start_date}
            ...
            case "skills":
              ...
              return (
                <section key="skills" className="page-break-avoid">
                  <h2 className="text-[10.5pt] font-bold uppercase tracking-wider text-zinc-950 border-b border-zinc-400 pb-0.5 mb-1.5 font-sans">
                    {labels.skills}
                  </h2>
                  <div className="space-y-1 text-[9.5pt]">
                    {skills.map((cat) => (
                      <div key={cat.id || cat.category} className="leading-snug">
                        <span className="font-bold text-zinc-900 font-sans text-[9pt] mr-1">
                          {cat.category}:
                        </span>
                  ...
                  <div className="space-y-1 text-[9.5pt]">
                    {certifications.map((cert) => (
                      <div key={cert.id} className="flex justify-between items-baseline">
                        ...
                        {cert.date && (
                          <span className="text-[8.5pt] italic text-zinc-600 font-sans">
                            {cert.date}
                          </span>
                        )}
                      </div>
              ...
              {sec.entries.map((entry) => (
                <div key={entry.id} className="leading-snug">
                  <div className="flex justify-between items-baseline">
                    ...
                    {entry.date && <span className="text-[8.5pt] font-sans text-zinc-600">{entry.date}</span>}
                  </div>
